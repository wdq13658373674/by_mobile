...
(function($, document) {
	...
	var DtPicker = $.DtPicker = $.Class.extend({
		init: function(options) {
			 ...
			 *自定义修改
			 * */
			if(document.getElementById('by-date-picker') && document.getElementById('by-date-picker').children.length == 0){
                document.getElementById('by-date-picker').appendChild(_picker);
            }else {
			 ...
			 * */
			if(ui.finish){
                ui.finish.addEventListener('tap', function() {
                    if(self.options){
                        var rs = self.callback(self.getSelected());
                    }else {
                        return;
		...
		getSelected: function() {
			...
			switch (type) {
				...
				case 'hour':
					...
					break;
                case 'day':
                    selected.value = selected.m.value + '-' + selected.d.value ;
                    selected.text = selected.m.text + '-' + selected.d.text;
                    break;
			}
			return selected;
		},
